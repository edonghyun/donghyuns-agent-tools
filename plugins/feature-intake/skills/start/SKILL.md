...
2. Inspect
   - Apply `feature-intake:inspect` to inventory structure, pages, controls, external calls, state, data, and dead paths.

3. Capture
   - Apply `feature-intake:capture` to collect page, interaction, dialog, edge-state, and contact-sheet screenshots.
...
5. Analyze
   - Apply `feature-intake:analyze` to write user flows, journeys, use cases, inventories, and screen annotations.
...
- User flow, user journeys, use cases, screen-by-screen annotations, integration fit, risks, and MVP/backlog exist.
